...
{
    sealed class SurfaceWithInfo : IDisposable
    {
    ...
    sealed class CaptureFrameWait : IDisposable
    {
        ...
        private void Cleanup()
        {
            using (var multithreadLock = new MultithreadLock(_multithread))
            {
                ...
                while (capturedFrames.TryDequeue(out Direct3D11CaptureFrame frame))
                    frame.Dispose();
            }
        }
        ...
        SurfaceWithInfo ToSurfaceWithInfo(Direct3D11CaptureFrame frame)
        {
            ...
            using (var sourceTexture = Direct3D11Helpers.CreateSharpDXTexture2D(frame.Surface))
            {
                ...
                {
                    var width = Clamp(frame.ContentSize.Width, 0, frame.Surface.Description.Width);
                    var height = Clamp(frame.ContentSize.Height, 0, frame.Surface.Description.Height);
        ...
        public SurfaceWithInfo GetNextFrame()
        {
            if (capturedFrames.TryDequeue(out Direct3D11CaptureFrame frame))
                using (frame)
                    return ToSurfaceWithInfo(frame);
            ...
            if (!capturedFrames.TryDequeue(out frame))
                throw new InvalidOperationException("Can't deque concurrent collection");
        ...
        private ConcurrentQueue<Direct3D11CaptureFrame> capturedFrames = new ConcurrentQueue<Direct3D11CaptureFrame>();
